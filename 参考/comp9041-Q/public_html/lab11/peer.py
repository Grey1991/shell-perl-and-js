...
def check_result():
	...
	# print(type(zid))
	# print (os.getcwd())
	files = glob.glob(r''+ASSESSMENTS_DIRECTORY+'/'+zid +'*')
	student_details = read_student_details()
	...
	for fi in files:
		with open(fi) as f:
			grade = f.readline()
			grades.append(grade)
		accessor_id = fi.split('.')[1]
		assessments.append({'id':accessor_id,'name':student_details[accessor_id]['name'],'grade':grade})
	grades = sorted(grades)
	# print(grades)
	# print(assessments)
	if len(grades)%2 == 1:
		medium = grades[len(grades)//2]
		# medium = 'medium'
	else:
		medium = grades[len(grades)//2 - 1] + '/' + grades[len(grades)//2]
		# medium = 'medium'
