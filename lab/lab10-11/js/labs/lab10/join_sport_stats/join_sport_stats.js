...
function makeTeamList(teamData, namesData, teamsData) {
    ...
    let result = [];
    let t_name = teamsData.filter(x => x.id === teamData.team.id).map(x=>x.team);
    result.push(`${t_name}, coached by ${teamData.team.coach}`);

    teamData.players.sort((a,b) => parseInt(a.matches) < parseInt(b.matches));
    for (let i=0; i< teamData.players.length; i++){
        let p_name = namesData.filter(name => name.id === teamData.players[i].id).map(x=>x.name);
        result.push(`${i+1}. ${p_name}`);
    }
    return result;
}
